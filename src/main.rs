...
fn print_char_down(ch: char, (y_init, y_end) : (u16, u16), x: u16, stdout :  Arc<Mutex<Stdout>>)
{
    let mut stdout = stdout.lock().unwrap(); // Vérouillage de stdout
...
fn print_char_up(ch: char, (y_init, y_end) : (u16, u16), x: u16, stdout :  Arc<Mutex<Stdout>>)
{
    let mut stdout = stdout.lock().unwrap(); // Vérouillage de stdout
...
fn print_char_to_right(ch: char, (x_init, x_end) : (u16, u16), y : u16, stdout :  Arc<Mutex<Stdout>>)
{
    let mut stdout = stdout.lock().unwrap(); // Vérouillage de stdout
...
//Affiche un caractère qui part de (x_init, y) à (x_end, y)
fn print_char_to_left(ch: char, (x_init, x_end) : (u16, u16), y : u16, stdout :  Arc<Mutex<Stdout>>)
{
    let mut stdout = stdout.lock().unwrap(); // Vérouillage de stdout
...
fn print_char(ch: char, x : u16, y : u16, direction : &str, stdout : Arc<Mutex<Stdout>>, height : u16, width : u16)
{
...
fn print_string(string: &str, x : u16, y : u16, direction : &str, stdout :  Arc<Mutex<Stdout>>, height : u16, width : u16)
{
    ...
    for (i, ch) in string.chars().enumerate() {
        ...
        let handle = thread::spawn(move || {
            print_char(ch, x + i, y, &direction_clone, stdout_clone, height, width);
...
fn main() 
{
    let stdout = Arc::new(Mutex::new(std::io::stdout())); // stdout sécurisé avec Mutex
    let (width, height) = terminal::size().unwrap(); // On récupère la hauteur et la largeur du terminal

    print_string("abcdef", 5, 5, "down", stdout.clone(), height, width);
